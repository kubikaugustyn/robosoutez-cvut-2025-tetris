...
from augustyn_tetris_robot.fake_np.polygons import Rectangle
...
def render(environment: Environment, goals: Goals) -> MatLike:
    ...
    pt: Vec2 = environment.robot.color_sensor.copy_rotated(Vec2(0,0),environment.robot.rotation) + environment.robot.translation + translation
    ...
    pt: Vec2 = environment.robot.wheels[0].copy_rotated(Vec2(0,0),environment.robot.rotation) + environment.robot.translation + translation
    cv2.circle(img, (int(pt.x), int(pt.y)), 5, (255, 0, 0), -1)
    pt: Vec2 = environment.robot.wheels[1].copy_rotated(Vec2(0,0),environment.robot.rotation) + environment.robot.translation + translation
    cv2.circle(img, (int(pt.x), int(pt.y)), 5, (255, 0, 0), -1)
